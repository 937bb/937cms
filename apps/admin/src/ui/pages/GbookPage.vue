  ...
  <n-space vertical size="large">
    <n-card title="留言板管理">
      <template #header-extra>
        <n-space>
          <n-select v-model:value="statusFilter" :options="statusOptions" placeholder="状态" clearable style="width: 100px" @update:value="search" />
        </n-space>
      </template>
...
const columns: DataTableColumns<any> = [
  ...
  { title: '用户', key: 'nickname', width: 100, render: (row) => row.nickname || row.username || row.name || '匿名' },
  ...
  { title: '回复', key: 'reply', ellipsis: { tooltip: true } },
  {
    ...
    key: 'status',
    width: 80,
    render: (row) => h(NTag, { type: row.status ? 'success' : 'warning', size: 'small' }, () => (row.status ? '已通过' : '待审核')),
  },
  {
    title: '操作',
    key: 'actions',
    ...
    render: (row) =>
      h(NSpace, { size: 'small' }, () => [
        ...
        h(NButton, { size: 'small', type: row.status ? 'warning' : 'success', onClick: () => toggleStatus(row) }, () => (row.status ? '禁用' : '通过')),
