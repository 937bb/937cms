  ...
  <n-space vertical size="large">
    <n-card :title="filterPreset ? `视频管理 - ${filterPreset}` : '视频管理'">
      ...
      <n-space align="center" wrap style="margin-bottom: 12px">
        <n-input v-model:value="filters.keyword" placeholder="搜索片名" style="width: 240px" clearable />
        <n-tree-select
        ...
        <n-tabs type="line" animated>
            <n-tab-pane name="basic" tab="基本">
              ...
              <n-gi :span="2">
                <n-form-item label="分类">
                  <n-tree-select v-model:value="form.type_id" :options="typeTreeOptions" placeholder="请选择分类" />
                </n-form-item>
              </n-gi>
              ...
              <n-gi>
                <n-form-item label="锁定/版权">
                  <n-space>
                    <n-switch v-model:value="form.vod_lock" :checked-value="1" :unchecked-value="0" />
                    <n-switch v-model:value="form.vod_copyright" :checked-value="1" :unchecked-value="0" />
                  </n-space>
                </n-form-item>
              </n-gi>
              ...
              <n-gi :span="2">
                <n-form-item label="简介">
                  <n-input v-model:value="form.vod_content" type="textarea" :autosize="{ minRows: 4, maxRows: 10 }" />
                </n-form-item>
              </n-gi>
            ...
            <n-tab-pane name="images" tab="图片">
              <n-form :model="form" label-placement="left" label-width="130">
                ...
                <n-form-item label="截图(文本)">
                  ...
                  />
                  <template #feedback>可填多张图片地址，格式按 MacCMS 约定（如用换行/分隔符）。</template>
                </n-form-item>
              </n-form>
            </n-tab-pane>
            ...
            <n-tab-pane name="play" tab="播放">
              <div style="padding: 12px 0">
                <div v-if="!form.playList || form.playList.length === 0" style="color: #999; text-align: center; padding: 40px 0">
                  暂无播放源
                </div>
                <div v-else>
                  <div v-for="(source, idx) in form.playList" :key="idx" style="margin-bottom: 24px; padding: 12px; border: 1px solid #e5e7eb; border-radius: 4px">
                    <div style="font-weight: 500; margin-bottom: 12px">{{ source.playerName }}</div>
                    <div v-if="!source.episodes || source.episodes.length === 0" style="color: #999; padding: 12px 0">
                      暂无剧集
                    </div>
                    <div v-else style="max-height: 200px; overflow-y: auto; border: 1px solid #f0f0f0; border-radius: 2px; padding: 8px">
                      <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 8px">
                        <div v-for="(ep, epIdx) in source.episodes" :key="epIdx" style="padding: 8px; background: #f5f5f5; border-radius: 2px; font-size: 12px; cursor: pointer; transition: background 0.2s" @click="editingEpisode = { ...ep, sourceIdx: idx, epIdx }; showEpisodeEdit = true" @mouseenter="$event.target.style.background = '#e8e8e8'" @mouseleave="$event.target.style.background = '#f5f5f5'">
                          <div style="font-weight: 500">第{{ ep.num }}集</div>
                          <div style="color: #666; margin-top: 4px; word-break: break-all">{{ ep.title || ep.url }}</div>
                        </div>
                        <div style="padding: 8px; background: #f0f0f0; border-radius: 2px; font-size: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background 0.2s" @click="addEpisode(idx)" @mouseenter="$event.target.style.background = '#e0e0e0'" @mouseleave="$event.target.style.background = '#f0f0f0'">
                ...
                <n-form :model="form" label-placement="left" label-width="130" style="margin-top: 24px; padding-top: 24px; border-top: 1px solid #e5e7eb">
            ...
            <n-tab-pane name="down" tab="下载">
              <n-form :model="form" label-placement="left" label-width="130">
                ...
                <n-form-item label="下载地址(url)">
                  <n-input v-model:value="form.vod_down_url" type="textarea" :autosize="{ minRows: 6, maxRows: 16 }" />
                </n-form-item>
            ...
            <n-tab-pane name="advanced" tab="更多">
              ...
              <n-gi>
                <n-form-item label="日/周/月点击">
                  <n-input-number v-model:value="form.vod_hits_day" :min="0" :max="1_000_000_000" />
                  <div style="width: 8px" />
                  <n-input-number v-model:value="form.vod_hits_week" :min="0" :max="1_000_000_000" />
                  <div style="width: 8px" />
                  <n-input-number v-model:value="form.vod_hits_month" :min="0" :max="1_000_000_000" />
                </n-form-item>
              </n-gi>
              <n-gi>
              ...
              <n-gi>
                <n-form-item label="评分总/人数">
                  <n-input-number v-model:value="form.vod_score_all" :min="0" :max="1_000_000_000" />
                  <div style="width: 8px" />
                  <n-input-number v-model:value="form.vod_score_num" :min="0" :max="1_000_000_000" />
                </n-form-item>
              </n-gi>

              <n-gi>
                <n-form-item label="豆瓣ID">
                  <n-input-number v-model:value="form.vod_douban_id" :min="0" :max="1_000_000_000" />
                </n-form-item>
              </n-gi>
              <n-gi>
                <n-form-item label="豆瓣评分">
                  <n-input-number v-model:value="form.vod_douban_score" :min="0" :max="10" :step="0.1" />
                </n-form-item>
              </n-gi>
            ...
            <n-tab-pane name="plot" tab="分集剧情">
              <n-form :model="form" label-placement="left" label-width="130">
                ...
                <n-form-item label="剧情标题">
                  <n-input v-model:value="form.vod_plot_name" type="textarea" :autosize="{ minRows: 4, maxRows: 10 }" />
                </n-form-item>
                <n-form-item label="剧情详情">
                  <n-input v-model:value="form.vod_plot_detail" type="textarea" :autosize="{ minRows: 6, maxRows: 16 }" />
                </n-form-item>
              </n-form>
            </n-tab-pane>
        </n-tabs>
      ...
      <n-card title="编辑剧集" closable style="width: 500px" @close="showEpisodeEdit = false">
        <n-form :model="editingEpisode" label-placement="left" label-width="100">
          ...
          <n-form-item label="播放地址">
            <n-input v-model:value="editingEpisode.url" type="textarea" :autosize="{ minRows: 4, maxRows: 8 }" placeholder="播放地址" />
          </n-form-item>
        </n-form>
        <template #footer>
          <n-space justify="end">
...
import type { DataTableColumns, DataTableRowKey, PaginationProps, TreeSelectOption } from 'naive-ui';
...
const filters = reactive<{ keyword: string; typeId: number | null; status: number | null; lock: number | null; url: string; points: string; plot: number | null }>({
  keyword: '',
  typeId: null,
  status: null,
  lock: null,
...
function initFiltersFromRoute() {
  const q = route.query;
  if (q.filter === 'nourl') filters.url = '0';  // 0=无地址
      ...
      for (const source of form.playList) {
        if (source.episodes && source.episodes.length > 0) {
          for (const ep of source.episodes) {
            if (ep.id) {
              // 更新现有剧集
              await http.post(`/admin/vods/${form.vod_id}/episodes/${ep.id}`, {
                episode_num: ep.num,
                title: ep.title,
                url: ep.url,
              }).catch(() => void 0);
              ...
              // 创建新剧集
              await http.post(`/admin/vods/${form.vod_id}/episodes/create`, {
                source_id: source.id,
                episode_num: ep.num,
                title: ep.title,
                url: ep.url,
...
async function batchEnable() {
  ...
  try {
    await http.post('/admin/vods/batch-update-field', { ids: selectedIds.value, field: 'vod_status', value: 1 });
    msg.success('批量上架成功');
    await load();
  } catch (e: any) {
...
async function batchDisable() {
  if (selectedIds.value.length === 0) return;
  try {
    await http.post('/admin/vods/batch-update-field', { ids: selectedIds.value, field: 'vod_status', value: 0 });
    msg.success('批量下架成功');
    await load();
  } catch (e: any) {
    msg.error(String(e?.response?.data?.message || e?.message || '操作失败'));
...
const columns: DataTableColumns<VodRow> = [
  ...
  { title: '分类', key: 'type_name', width: 160, render: (r) => `${r.type_id} - ${r.type_name || ''}` },
  ...
  { title: '状态', key: 'vod_status', width: 90, render: (r) => statusTag(Number(r.vod_status)) },
  {
    ...
    render: (row) =>
      h(
        ...
        [
          h(NButton, { size: 'small', tertiary: true, onClick: () => openEdit(row.vod_id) }, { default: () => '编辑' }),
          h(
            NPopconfirm,
