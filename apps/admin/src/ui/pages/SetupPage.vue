<template>
  <div class="bb-setup">
    <n-card title="初始化 937 CMS（首次启动）" style="max-width: 860px; width: 100%">
      <!-- 目录权限检测 -->
      <n-alert v-if="hasDirectoryError" type="error" title="目录权限错误" style="margin-bottom: 12px">
        <div v-for="dir in status.directories?.filter(d => !d.writable)" :key="dir.path">
          {{ dir.path }}: {{ dir.error || '无写入权限' }}
        </div>
      </n-alert>

      <n-alert v-if="status.configured && status.needsRestart" type="warning" title="已写入配置，但需要重启 API" style="margin-bottom: 12px">
        由于当前 API 进程启动时还未加载数据库配置，请停止并重新启动 API，然后再回到此页面点击「我已重启，去登录」。
      </n-alert>

      <n-alert v-else-if="status.configured" type="success" title="已初始化" style="margin-bottom: 12px">
        系统已完成初始化，可直接前往登录。
      </n-alert>

      <n-form ref="formRef" :model="model" :rules="rules" label-placement="left" label-width="150" size="medium">
        ...
        <n-form-item label="密码" path="mysqlPassword">
          <n-input v-model:value="model.mysqlPassword" type="password" show-password-on="click" placeholder="请输入" />
        </n-form-item>
        <n-form-item label="自动创建数据库" path="mysqlCreateDatabase">
          <n-switch v-model:value="model.mysqlCreateDatabase" />
        </n-form-item>
        <n-form-item label=" ">
          <n-button :loading="testingMysql" type="primary" @click="onTestMysql">测试 MySQL 连接</n-button>
          <n-text v-if="mysqlTested" depth="3" style="margin-left: 12px; color: #18a058">✓ 连接成功</n-text>
        </n-form-item>
      ...
      <n-space justify="end">
        <n-button v-if="status.configured && !status.needsRestart" secondary @click="goLogin">去登录</n-button>
        <n-button v-else-if="status.configured && status.needsRestart" secondary @click="goLogin">我已重启，去登录</n-button>
        <n-button v-else type="primary" :loading="loading" :disabled="!mysqlTested" @click="onSubmit">开始初始化</n-button>
      </n-space>

      <n-divider />
      <n-space vertical>
        <n-text depth="3">提示：初始化完成后，站外入库密码会由系统随机生成，可在「系统设置」里查看/修改。</n-text>
      </n-space>
    </n-card>
  </div>
</template>
...
import { clearSetupStatusCache, getSetupStatusCached, type SetupStatus } from '../../lib/setup-status';
...
const rules: FormRules = {
  ...
  mysqlPort: [{ required: true, type: 'number', message: '请输入 MySQL 端口', trigger: ['blur', 'change'] }],
  ...
  adminPassword: [{ required: true, message: '请输入管理员密码（至少 6 位）', trigger: ['blur', 'input'] }],
};
