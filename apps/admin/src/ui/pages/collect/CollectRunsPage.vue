...
const jobOptions = computed(() => jobs.value.map((j) => ({ label: `${j.id} - ${j.name}`, value: j.id })));
...
const columns: DataTableColumns<RunItem> = [
  ...
  { title: '任务', key: 'job_name', minWidth: 200, render: (r) => `${r.job_id} - ${r.job_name || ''}` },
  ...
  { title: '结束时间', key: 'finished_at', width: 180, render: (r) => formatTime(r.finished_at) },
  {
    ...
    render: (row) =>
      h(
        NButton,
        { text: true, type: row.error_count > 0 ? 'error' : 'default', onClick: () => openDetail(row) },
      ...
      if (row.status === 0 || row.status === 1) {
        btns.push(
          h(
            ...
            {
              trigger: () => h(NButton, { size: 'small', tertiary: true, type: 'warning' }, { default: () => '取消' }),
              default: () => '确认取消该任务？',
            },
          ),
        );
  ...
  <n-space vertical size="large">
    <n-card title="采集记录（Runs）">
      <n-space align="center" wrap style="margin-bottom: 12px">
        <n-select v-model:value="filters.status" :options="statusOptions" clearable placeholder="状态" style="width: 180px" />
        <n-select v-model:value="filters.jobId" :options="jobOptions" clearable placeholder="任务" style="min-width: 260px" />
    ...
    <n-modal v-model:show="showDetail" preset="card" style="width: min(1000px, 95vw)">
      <template #header>
        <div v-if="detail">
          采集任务 #{{ detail.id }} - {{ detail.job_name }}
        </div>
      </template>
      <n-space vertical size="large">
        <!-- 总体统计 -->
        <n-space justify="space-between" align="center" style="padding: 16px; background: #f5f5f5; border-radius: 4px">
          ...
          <n-statistic label="总进度" :value="`${detail?.progress_page}/${detail?.progress_total_pages}`" />
          ...
          <n-statistic label="错误数" :value="detail?.error_count" :type="detail?.error_count ? 'error' : 'success'" />
        </n-space>
        ...
        <div v-if="taskDetails.length > 0">
          ...
          <n-space vertical style="width: 100%">
            <div v-for="task in taskDetails" :key="task.id" style="padding: 12px; border: 1px solid #e0e0e0; border-radius: 4px">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px">
                <div style="font-weight: 600">{{ task.source_name || `源 #${task.source_id}` }}</div>
                <div style="display: flex; gap: 8px; align-items: center">
                  ...
                  <n-button v-if="task.status === 3 || task.status === 0" size="small" type="primary" tertiary @click="openResumeModal(task.source_id, detail?.id || 0)">断点续采</n-button>
                </div>
              </div>
              <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 12px; font-size: 12px">
                <div><span style="color: #999">进度:</span> {{ task.current_page }}/{{ task.total_pages }}</div>
                <div><span style="color: #999">采集数:</span> {{ (task.created_count || 0) + (task.updated_count || 0) }}</div>
              ...
              <div v-if="task.error_message" style="margin-top: 8px; padding: 8px; background: #fff3cd; border-radius: 2px; font-size: 12px; color: #856404">
                {{ task.error_message }}
              </div>
            </div>
          </n-space>
    ...
    <n-modal v-model:show="showResumeModal" preset="card" title="断点续采确认" style="max-width: 400px; width: 100%">
      <div style="padding: 12px 0">
        <div style="margin-bottom: 16px">确认从源 #{{ selectedResumeSource }} 的断点继续采集？</div>
        <div style="font-size: 12px; color: #999; margin-bottom: 16px">系统将从该源上次中断的位置继续采集</div>
      </div>
      <n-space justify="end">
