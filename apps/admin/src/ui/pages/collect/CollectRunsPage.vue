...
import { NButton, NPopconfirm, NTag, NStatistic, useMessage } from 'naive-ui';
...
function openDetail(row: RunItem) {
  detail.value = row;
...
const columns: DataTableColumns<RunItem> = [
  ...
  { title: '状态', key: 'status', width: 100, render: (r) => statusTag(Number(r.status)) },
  {
    ...
    key: 'progress',
    width: 140,
    render: (r) => `${Number(r.progress_page || 0)}/${Number(r.progress_total_pages || 0)}`,
  },
  ...
  { title: 'worker', key: 'worker_id', width: 140 },
  {
    title: '消息',
    key: 'message',
    minWidth: 240,
...
<template>
  <n-space vertical size="large">
    ...
    <n-modal v-model:show="showDetail" preset="card" style="width: min(900px, 95vw)">
      <template #header>
        <div v-if="detail">
      ...
      <n-space vertical size="large">
        <n-space justify="space-between" align="center" style="padding: 16px; background: #f5f5f5; border-radius: 4px">
          ...
          <n-statistic label="进度" :value="`${detail?.progress_page}/${detail?.progress_total_pages}`" />
        ...
        <n-code v-if="detail" :code="detail.message || ''" language="text" />
      </n-space>
    </n-modal>
  </n-space>
</template>
