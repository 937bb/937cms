  ...
  <n-space vertical size="large">
    <n-card title="演员库">
      ...
      <n-space align="center" wrap style="margin-bottom: 12px">
        <n-input v-model:value="filters.keyword" placeholder="搜索演员名" style="width: 200px" clearable />
    ...
    <n-modal v-model:show="showModal">
      <n-card :title="form.actor_id ? '编辑演员' : '添加演员'" closable style="width: 800px" @close="showModal = false">
          ...
          <n-grid :cols="2" :x-gap="12">
            ...
            <n-gi>
              <n-form-item label="性别">
                <n-select v-model:value="form.actor_sex" :options="[{ label: '男', value: '男' }, { label: '女', value: '女' }]" clearable />
              </n-form-item>
            </n-gi>
            <n-gi>
            ...
            <n-gi :span="2">
              <n-form-item label="详细介绍">
                <n-input v-model:value="form.actor_content" type="textarea" :autosize="{ minRows: 4, maxRows: 10 }" />
              </n-form-item>
            </n-gi>
          </n-grid>
        </n-form>
...
type ActorRow = { actor_id: number; actor_name: string; actor_en: string; actor_pic: string; actor_sex: string; actor_area: string; actor_status: number; actor_level: number; actor_hits: number };
...
const form = reactive<Record<string, any>>({
  actor_id: 0, actor_name: '', actor_en: '', actor_alias: '', actor_status: 1, actor_letter: '', actor_sex: '',
  actor_pic: '', actor_blurb: '', actor_area: '', actor_height: '', actor_weight: '', actor_birthday: '',
  actor_starsign: '', actor_works: '', actor_level: 0, actor_content: '',
});

async function load() {
  loading.value = true;
  try {
    const res = await http.get('/admin/actors', { params: { page: page.value, pageSize: pageSize.value, keyword: filters.keyword || undefined, status: filters.status ?? undefined } });
...
function openAdd() {
  Object.assign(form, { actor_id: 0, actor_name: '', actor_en: '', actor_alias: '', actor_status: 1, actor_letter: '', actor_sex: '', actor_pic: '', actor_blurb: '', actor_area: '', actor_height: '', actor_weight: '', actor_birthday: '', actor_starsign: '', actor_works: '', actor_level: 0, actor_content: '' });
...
async function openEdit(id: number) {
  try {
    ...
    Object.assign(form, {
      actor_id: item.actor_id || 0, actor_name: item.actor_name || '', actor_en: item.actor_en || '', actor_alias: item.actor_alias || '',
      actor_status: Number(item.actor_status) || 0, actor_letter: item.actor_letter || '', actor_sex: item.actor_sex || '',
      actor_pic: item.actor_pic || '', actor_blurb: item.actor_blurb || '', actor_area: item.actor_area || '',
      actor_height: item.actor_height || '', actor_weight: item.actor_weight || '', actor_birthday: item.actor_birthday || '',
      actor_starsign: item.actor_starsign || '', actor_works: item.actor_works || '', actor_level: Number(item.actor_level) || 0,
...
async function save() {
  if (!form.actor_name) { msg.warning('请输入演员名'); return; }
  saving.value = true;
  try {
...
async function remove(id: number) {
  try { await http.post('/admin/actors/delete', { actor_id: id }); msg.success('已删除'); await load(); }
...
async function batchEnable() {
  ...
  try { await http.post('/admin/actors/batch-update-status', { ids: selectedIds.value, status: 1 }); msg.success('批量启用成功'); await load(); }
  catch (e: any) { msg.error(String(e?.response?.data?.message || e?.message || '操作失败')); }
}

async function batchDisable() {
  if (!selectedIds.value.length) return;
  try { await http.post('/admin/actors/batch-update-status', { ids: selectedIds.value, status: 0 }); msg.success('批量禁用成功'); await load(); }
  catch (e: any) { msg.error(String(e?.response?.data?.message || e?.message || '操作失败')); }
}

async function batchDelete() {
  if (!selectedIds.value.length) return;
  try { await http.post('/admin/actors/batch-delete', { ids: selectedIds.value }); msg.success('批量删除成功'); await load(); }
  catch (e: any) { msg.error(String(e?.response?.data?.message || e?.message || '操作失败')); }
}
...
const columns: DataTableColumns<ActorRow> = [
  ...
  { title: '状态', key: 'actor_status', width: 80, render: (r) => statusTag(r.actor_status) },
  {
    ...
    render: (row) => h('div', { style: 'display:flex; gap:8px;' }, [
      h(NButton, { size: 'small', tertiary: true, onClick: () => openEdit(row.actor_id) }, { default: () => '编辑' }),
      h(NPopconfirm, { onPositiveClick: () => remove(row.actor_id) }, { trigger: () => h(NButton, { size: 'small', tertiary: true, type: 'error' }, { default: () => '删除' }), default: () => '确认删除？' }),
    ]),
  },
];
