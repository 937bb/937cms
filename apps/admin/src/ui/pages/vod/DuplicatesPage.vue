...
type VideoItem = { vod_id: number; vod_name: string; type_id: number; vod_time: number; vod_status: number };
type DuplicateItem = { name: string; count: number; videos: VideoItem[] };
type TableRow = { key: string; isGroup: boolean; name: string; count?: number; vod_id?: number; type_id?: number; vod_time?: number; vod_status?: number; videos?: VideoItem[] };
...
const tableData = computed<TableRow[]>(() => {
  ...
  for (const item of items.value) {
    // 添加分组行
    rows.push({ key: `group-${item.name}`, isGroup: true, name: item.name, count: item.count, videos: item.videos });
    // 添加视频行
    for (const v of item.videos) {
      rows.push({ key: `video-${v.vod_id}`, isGroup: false, name: item.name, vod_id: v.vod_id, type_id: v.type_id, vod_time: v.vod_time, vod_status: v.vod_status });
    }
  }
  return rows;
});
...
async function load() {
  loading.value = true;
  try {
    const res = await http.get('/admin/vods/duplicates', { params: { page: page.value, pageSize: pageSize.value } });
...
const columns: DataTableColumns<TableRow> = [
  ...
  },
  {
    ...
    key: 'count',
    width: 100,
    render: (row) => row.isGroup ? h(NTag, { type: 'warning' }, { default: () => `${row.count}个` }) : `分类: ${row.type_id}`,
  },
  {
    title: '更新时间',
    key: 'vod_time',
  ...
  },
  {
    ...
    render: (row) => {
      ...
      return Number(row.vod_status) ? h(NTag, { type: 'success', size: 'small' }, { default: () => '上架' }) : h(NTag, { type: 'default', size: 'small' }, { default: () => '下架' });
    },
  },
  {
    title: '操作',
    ...
    render: (row) => {
      if (row.isGroup) {
        return h(NSpace, { size: 'small' }, {
          default: () => [
            h(NPopconfirm, { onPositiveClick: () => keepFirst(row.videos!) }, {
              trigger: () => h(NButton, { size: 'small', type: 'warning' }, { default: () => '保留第一个' }),
              default: () => `删除其他 ${(row.count || 0) - 1} 个？`,
            }),
            h(NPopconfirm, { onPositiveClick: () => keepLast(row.videos!) }, {
              trigger: () => h(NButton, { size: 'small', type: 'warning' }, { default: () => '保留最后一个' }),
              default: () => `删除其他 ${(row.count || 0) - 1} 个？`,
            }),
          ],
        });
      }
      return h(NPopconfirm, { onPositiveClick: () => deleteVideos([row.vod_id!]) }, {
